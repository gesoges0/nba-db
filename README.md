...
mysql> SELECT t.full_name, tgl.* FROM team_game_log AS tgl INNER JOIN teams AS t ON tgl.Team_ID=t.id WHERE tgl.WL="W" AND tgl.PTS = (SELECT MIN(tgl1.PTS) FROM team_game_log AS tgl1 WHERE tgl1.WL = "W");
+------------+------------+------------+--------------+-----------+------+------+------+-------+------+------+------+--------+------+------+---------+------+------+--------+------+------+------+------+------+------+------+------+------+---------------------+---------------------+
